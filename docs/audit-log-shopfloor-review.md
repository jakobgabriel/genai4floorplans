...
| C-12 | Pattern-Library | 🔵 offen (Roadmap) | §30–35; hängt an C-10 (jetzt erfüllt) — Auto-Extraktion aus Snapshots als nächster Schritt |
...
**Test-/Typecheck-Stand nach Umsetzung:** `npm run typecheck` grün (core+web+server); `npm test` **538 grün / 2 skipped** (frischer Checkout ohne Handgriff dank B-01). Golden-Fixtures bewusst aktualisiert, wo eine Kennzahl korrigiert wurde; jeder Fix ist durch neue Tests abgesichert.
