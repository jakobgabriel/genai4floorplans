...
| **C-11** | Portfolio / Teilenummer-Matrix | ✅ Gate 1–3 | `Model.parts` (Teilenummer = abstrakter Capability-Bedarf, §1.1-konform); `portfolioMatrix` (Gate 1: Teil × Capability, provided/Alternative/fehlend, blockierende Capabilities), `portfolioCapacity` (**Gate 2** Volumenband, **Gate 3** Kapazität = Bearbeitung + Rüsten/Kampagnen vs verfügbare Zeit, **Drop-Analyse** = billigste zu droppende Teile). Analyse-Tab „Portfolio" mit Matrix, Auslastungs-Gate, Drop-Vorschlag, Teile-Editor (Bedarf/Kampagnen/Familie); Inspector `provides`-Editor |
| **C-11 Gate 4** | Balance-Gate (§18 Gate 4) | ✅ | `engine/gate4.ts` `gate4Balance`: kann die Arbeit auf die (feste) Stationszahl der Linie balanciert werden? Zwei Versagensarten — unteilbares Element > Takt (muss geteilt/automatisiert/parallelisiert werden) **oder** weniger Stationen als das theoretische Minimum (Σ Arbeit ÷ Takt). Gegen den **schwersten Modus** balanciert (nicht den Mix-Schnitt). Fließt in das Testfit-Balance-Gate ein |
| **C-04** | Testfit-Service (Machbarkeit ≠ Optimierung) | ✅ §20 | `engine/testfit.ts` `testfit(model)`: read-only Go/No-Go-Dienst, bewusst getrennt von der Optimierung. Aggregiert die Gates in fester Triage-Reihenfolge — Capability (Gate 1) → Takt → Balance (Gate 4) → Kapazität (Gate 3) → Layout-Realismus — und benennt die erste blockierende als **bindende Restriktion** (was zuerst zu beheben ist). Jede Kennzahl aus den bestehenden Engine-Modulen abgeleitet, daher nie im Widerspruch zu den Panels. Analyse-Dashboard: `FeasibilityBanner` über dem Rating (Verdikt, bindende Restriktion, Gate-Chips) |
| **C-09** | Messverteilungen (p50/p95/p99) | ✅ | `Station.cycleCV` (Variationskoeffizient σ/μ); `engine/variability.ts` `lineVariability`: je Station Lognormal-Zyklus → p50/p95/p99, Takt-Erreichungswahrscheinlichkeit P(Zyklus ≤ Takt), **fragile** Stationen (Mittel unter Takt, p95 darüber). Linien-Takterreichung ≈ Π der Stationswahrscheinlichkeiten (Unabhängigkeits-Näherung, ohne Puffer). Testfit-Takt-Gate wird **warn** statt pass, wenn der Engpass fragil ist. Analyse-Card „Cycle variability — the p95 tail", Inspector-Feld `Cycle CV`. Absent ⇒ 0 (deterministisch), Golden-Sample unverändert |
| **C-10** | Immutable Snapshots (Freigabe/Versionierung) | ✅ Increment 1 | `store/snapshots.ts`: eingefrorene, unveränderliche Modell-Kopien je Layout mit Zeitstempel, Label/Notiz und Lineage (`parentId` = Vorgänger-Version, §6). `engine/diff.ts` `modelDiff(a,b)`: strukturelle Differenz (Stationen hinzu/entfernt/geändert mit Feld-von→zu, Flows, Grid) + Zusammenfassung. Snapshots-Tab: Capture, Liste (neueste zuerst), Restore, Inline-Diff gegen aktuellen Stand inkl. Grade-Delta. Persistenz clientseitig (eigener localStorage-Store, da der Server-Tree-Reconcile nur bekannte Cell-Felder round-trippt) — Server-synchronisierte Releases = späteres Increment. Entpark-Voraussetzung für C-12 (Pattern-Library) |
| **C-12** | Pattern-Library (Auto-Extraktion) | ✅ Increment 1 | `engine/patterns.ts` `minePatterns(sources)`: schürft **wiederkehrende Prozess-Motive** (gerichtete Stationstyp-Ketten entlang Flows, Länge 2–4) über alle Layouts **und deren Snapshots** (die C-10 verfügbar macht) — Signatur, Vorkommen, Anzahl Quell-Layouts, konkrete Instanz-Stationen für die Extraktion; gerankt nach Breite der Wiederverwendung. Library-Seite „Recurring patterns": mined Kandidaten mit ×Vorkommen/Quellen und „save" → wird über bestehendes `makeSubflow`/`INSERT_SUBFLOW` (id-Remap) zu einem wiederverwendbaren Grouped Element. Baut auf der vorhandenen Subflow-Mechanik statt einen Parallel-Store zu schaffen |
| **D-01** | Doku veraltet | ✅ dieser Nachtrag | Status hier dokumentiert |
| **U-01** | UX-/Datenfluss-Sammelfix | ✅ | (1) **Fraktionale Operatoren**: `Station.operators` darf jetzt Bruchteile sein — ein Operator an mehreren Maschinen = z. B. 0.33 je Maschine, sodass Σ Operatoren die reale Kopfzahl statt „einer pro Station" ergibt; Durchsatz-Lanes runden (Goldens unverändert); Inspector-Feld mit `min 0 / step 0.1` + Erklärung. (2) **Zellformen** W (Doppel-U) und O (Loop) waren gebaut, aber nur der Optimizer erreichte sie — jetzt Palette-Buttons; Improved-Rationale benennt sie korrekt statt „straight I-line". (3) **Concept-Demand**: Schichtmodell/OEE/Arbeitstage sind jetzt Concept-Eigenschaft (`Concept.demand`), werden auf neue Layouts vererbt und vom aktiven Layout zurückgespiegelt → Analyse-Tab zeigt die im Concept definierten Werte statt Defaults. (4) **Schema-Tab** neu aus aktuellem `types.ts` (Version dynamisch aus `SCHEMA_VERSION`, alle Entitäten inkl. Demand/PartEntry/WorkElement/Capability und neue Station-Felder). (5) **CSS**: globale `svg`-Regel auf `.canvas-svg` eingegrenzt (Button-Icons/Dropdown-Pfeile hatten eine Box/verschwanden); Editor-Höhe `calc(100vh − 3rem)` statt `100vh`+Padding → kein Overscroll/schwarzer Bereich mehr |
| **U-01b** | Nachfassen: Overscroll + Concept-Demand härter gefixt | ✅ | Empirisch mit Playwright reproduziert. (a) **Overscroll/schwarzer Bereich:** Der `calc()+margin`-Ansatz maß auf Chromium zwar korrekt, ist aber margin-collapse-abhängig (schlägt je Browser fehl). Editor-Shell jetzt `position: fixed; inset: 3rem 0 0 0` → nichts Hohes bleibt im Fluss, die Seite kann **gar nicht** mehr scrollen (verifiziert: `scrollY` bleibt 0 nach Wheel), Sidebar scrollt intern weiter. (b) **Concept-Demand:** alle Pfade (Wizard→`generate.ts` stampt `brief.demand`, `createConcept`, `CapacityPanel` liest `model.demand`) verifiziert korrekt; zusätzliches Sicherheitsnetz: beim Öffnen eines Layouts **ohne** eigenes Demand wird es aus `Concept.demand` geseedet, damit Alt-Layouts/jeder Pfad die Concept-Werte im Analyse-Tab zeigen |
...
**Test-/Typecheck-Stand nach Umsetzung:** `npm run typecheck` grün (core+web+server); `npm test` **543 grün / 2 skipped** (frischer Checkout ohne Handgriff dank B-01). Golden-Fixtures bewusst aktualisiert, wo eine Kennzahl korrigiert wurde; jeder Fix ist durch neue Tests abgesichert.
...
| C-04 | Testfit-Service | Machbarkeit nicht von Optimierung getrennt (§16/§20 „single most important architectural decision") — keine Envelope-/Feasibility-Prüfung vor Optimierung | §16, §20 | ✅ (`engine/testfit.ts`, s. 0b) |
...
| C-09 | Messverteilungen | Alles Mittelwerte; keine p50/p95/p99 (§13-F7 „the p95 tail — where losses live — is invisible") | §13, §16 | ✅ (`engine/variability.ts`, s. 0b) |
| C-10 | Immutable Snapshots | Modelle werden in-place mutiert; keine `version`/`parent_version`, keine Rekonstruktion zum Freigabestand | §6 | ✅ Inc 1 (`store/snapshots.ts` + `engine/diff.ts`, s. 0b) |
...
| C-12 | Pattern-Library | §30–35 „highest-value mechanic" komplett ungebaut; hängt an C-10 (ohne Snapshots keine Auto-Extraktion) | §30–35 | ✅ Inc 1 (`engine/patterns.ts` + Library-Seite, s. 0b) |
