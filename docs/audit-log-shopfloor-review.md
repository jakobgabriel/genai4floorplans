...
| C-09/10/12 | Verteilungen, Snapshots, Pattern-Library | 🔵 offen (Roadmap) | Greenfield-Features; Umfang je Wochen; Halbausbau widerspräche §4/§5 |
...
**Test-/Typecheck-Stand nach Umsetzung:** `npm run typecheck` grün (core+web+server); `npm test` **518 grün / 2 skipped** (frischer Checkout ohne Handgriff dank B-01). Golden-Fixtures bewusst aktualisiert, wo eine Kennzahl korrigiert wurde; jeder Fix ist durch neue Tests abgesichert.
