...
const KEY = "flowplan_process_library";
...
export function useLibrary(): LibraryApi {
  ...
  const importCapabilities = useCallback(() => {
    ...
    const hints = CAPABILITY_HINTS.map((h) => {
      const tail = h.capabilityId.split(".").pop() ?? h.capabilityId;
      return { capabilityId: h.capabilityId, label: tail.charAt(0).toUpperCase() + tail.slice(1) };
    });
    const fresh = fromCapabilities(hints, (i) => newId("lib" + i)).filter(
