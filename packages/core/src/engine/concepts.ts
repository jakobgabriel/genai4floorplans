...
/** A concept id. The five below ship built in, but the catalog is editable —
 *  a planner can add their own concept — so this is a free-form string. */
...
/** A concept catalog indexed by kind — what the editable UI and the sweep read. */
...
export function rankConcepts(
  ...
  return Object.keys(catalog)
    ...
    .sort((a, b) => b.fit - a.fit || (catalog[a.kind].capexPerStation - catalog[b.kind].capexPerStation));
}
...
/** The default catalog under an editable name — the concepts page reads this
 *  when nothing has been customised. Same object as CONCEPTS. */
export const CONCEPT_DEFAULTS: ConceptCatalog = CONCEPTS;
