...
The docs are published as a static [Docusaurus](https://docusaurus.io/) site
(open-source, MIT) on **GitHub Pages**. It is generated automatically and never
drifts from the code:

The whole reference is generated **from the source code**:

- **Narrative pages** are read directly from this repo's `docs/` folder — the
  single source of truth — so editing a spec updates the site.
- **Core & Web API references** are generated by [TypeDoc](https://typedoc.org/)
  from the packages' JSDoc into `docs/api/{core,web}/` (git-ignored, rebuilt each run).
- **REST API reference** is rendered by [Redoc](https://github.com/Redocly/redocly-cli)
  from the server's OpenAPI document — built from the same zod schemas the routes
  validate against (`scripts/emit-openapi.ts` → `docs-site/static/rest-api/`).
- `.github/workflows/docs.yml` builds and deploys to Pages on every push to `main`.

The site is a **standalone package** in `docs-site/` (its own `node_modules`, no
`"type": "module"`) so Docusaurus's SSR build is isolated from the ESM monorepo
root — Docusaurus does not support `type: module`.

```bash
npm run docs:install   # install the docs site's own dependencies (once)
npm run docs:dev       # TypeDoc API + local Docusaurus dev server (hot reload)
npm run docs:build     # TypeDoc API + production build to docs-site/build
```

**One-time repo setup:** GitHub → *Settings → Pages → Build and deployment →
Source: **GitHub Actions***. The site then lives at
`https://jakobgabriel.github.io/genai4floorplans/`.
